...
def test_bookings_are_paginated_and_completed_jobs_are_locked(app,client):
    ...
    first=client.get("/bookings/");html=first.get_data(as_text=True);assert first.status_code==200;assert html.count("BOOK-PAGE-")==20;assert "BOOK-HISTORY-0001" not in html
    second=client.get("/bookings/?page=2");assert second.status_code==200;assert second.get_data(as_text=True).count("BOOK-PAGE-")==5
    history_page=client.get("/bookings/?view=history");assert "BOOK-HISTORY-0001" in history_page.get_data(as_text=True)
