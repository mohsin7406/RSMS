...
class Expense(db.Model):
    id=db.Column(db.Integer,primary_key=True); expense_date=db.Column(db.Date,nullable=False,index=True); category=db.Column(db.String(50),nullable=False,index=True); amount=db.Column(db.Numeric(12,2),nullable=False,default=Decimal('0')); repair_id=db.Column(db.Integer,db.ForeignKey('repair_order.id'),index=True); description=db.Column(db.String(255),nullable=False); payment_method=db.Column(db.String(30)); reference=db.Column(db.String(100)); status=db.Column(db.String(20),nullable=False,default='Approved',index=True); approved_by=db.Column(db.Integer,db.ForeignKey('user.id')); approved_at=db.Column(db.DateTime); created_by=db.Column(db.Integer,db.ForeignKey('user.id')); created_at=db.Column(db.DateTime,server_default=db.func.now(),nullable=False)
    repair=db.relationship('RepairOrder',backref=db.backref('expenses',lazy=True)); creator=db.relationship('User',foreign_keys=[created_by]); approver=db.relationship('User',foreign_keys=[approved_by])
