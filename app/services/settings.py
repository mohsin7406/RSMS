...
def option_allowed(group,value):
    return bool(value) and value in get_options(group)

def format_number(prefix_key,fallback_prefix,model,field,now):
    prefix=(get_setting(prefix_key,fallback_prefix) or fallback_prefix).strip().upper()
    today=now.strftime("%Y%m%d")
    stem=f"{prefix}-{today}"
    latest=model.query.filter(getattr(model,field).like(f"{stem}-%")).order_by(model.id.desc()).first()
    sequence=int(getattr(latest,field).rsplit("-",1)[-1])+1 if latest and getattr(latest,field).rsplit("-",1)[-1].isdigit() else 1
