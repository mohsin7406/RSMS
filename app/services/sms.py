...
def _template_for(event):
    ...
    return EVENT_TEMPLATES.get(event)
...
def notify_customer(event, repair, **values):
    template = _template_for(event)
    if not template or not getattr(repair, "customer", None):
        return {"ok": False, "skipped": True, "reason": "Template disabled or customer missing"}
    ...
    try:
        ...
        return {"ok": False, "skipped": True, "reason": f"Template variable missing: {exc.args[0]}"}
