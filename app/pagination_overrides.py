...
PER_PAGE = 20
REPAIR_HISTORY_STATUSES = ("Completed", "Delivered", "Cancelled")
...
def _pager(pagination, endpoint, **params):
    ...
    return {
        ...
        "prev_url": url_for(endpoint, page=pagination.prev_num, **base) if pagination.has_prev else None,
        "next_url": url_for(endpoint, page=pagination.next_num, **base) if pagination.has_next else None,
    }
...
def register_pagination_overrides(app):
    ...
    originals = {name: app.view_functions.get(name) for name in ("ops.expenses", "ops.stock")}
    ...
    def repairs_list_paginated():
        ...
        if view == "history":
            query = query.filter(RepairOrder.status.in_(REPAIR_HISTORY_STATUSES))
        else:
            view = "current"
            query = query.filter(~RepairOrder.status.in_(REPAIR_HISTORY_STATUSES))
        if q:
            term = f"%{q}%"
            query = query.filter(or_(Customer.name.ilike(term), RepairOrder.job_number.ilike(term), RepairOrder.device.ilike(term), RepairOrder.imei.ilike(term)))
        if status:
    ...
    def inventory_list_paginated():
        ...
        if stock == "low":
            query = query.filter(Part.quantity <= Part.reorder_level)
        elif stock == "out":
            query = query.filter(Part.quantity <= 0)
        pagination = query.order_by(Part.name).paginate(page=page, per_page=PER_PAGE, error_out=False)
        low_stock_count = Part.query.filter(Part.active.is_(True), Part.quantity <= Part.reorder_level).count()
    ...
    def purchases_paginated():
        page = max(request.args.get("page", 1, type=int), 1)
        pagination = Purchase.query.order_by(Purchase.purchase_date.desc(), Purchase.id.desc()).paginate(page=page, per_page=PER_PAGE, error_out=False)
    ...
    def suppliers_paginated():
        page = max(request.args.get("page", 1, type=int), 1)
        pagination = Supplier.query.order_by(Supplier.name).paginate(page=page, per_page=PER_PAGE, error_out=False)
        rows = [(s, *supplier_totals(s)) for s in pagination.items]
    ...
    def expenses_paginated():
        if request.method == "POST":
            return originals["ops.expenses"]()
        from datetime import date
        page = max(request.args.get("page", 1, type=int), 1)
        pagination = Expense.query.order_by(Expense.expense_date.desc(), Expense.id.desc()).paginate(page=page, per_page=PER_PAGE, error_out=False)
        repairs = RepairOrder.query.filter(RepairOrder.deleted_at.is_(None), ~RepairOrder.status.in_(REPAIR_HISTORY_STATUSES)).order_by(RepairOrder.id.desc()).limit(50).all()
    ...
    def stock_paginated():
        if request.method == "POST":
            return originals["ops.stock"]()
        page = max(request.args.get("page", 1, type=int), 1)
        pagination = StockAllocation.query.order_by(StockAllocation.id.desc()).paginate(page=page, per_page=PER_PAGE, error_out=False)
        parts = Part.query.filter_by(active=True).order_by(Part.name).all()
        repairs = RepairOrder.query.filter(RepairOrder.deleted_at.is_(None), ~RepairOrder.status.in_(REPAIR_HISTORY_STATUSES)).order_by(RepairOrder.id.desc()).limit(50).all()
        techs = User.query.filter_by(role="technician").order_by(User.email).all()
    ...
    def audit_paginated():
        page = max(request.args.get("page", 1, type=int), 1)
        pagination = AuditEvent.query.order_by(AuditEvent.id.desc()).paginate(page=page, per_page=PER_PAGE, error_out=False)
