...
DOORSTEP_TRANSITIONS = {"Pending": {"Approved", "Cancelled"}, "Approved": {"Cancelled"}, "Completed": set(), "Cancelled": set()}
WHATSAPP_WEB_DEFAULT = "Hi {customer_name}, your FixZone repair {job_number} for {device} is currently {status}."
...
def _validate_repair(values):
    ...
    if values["service_type"] == "Doorstep":
        if values["status"] not in DOORSTEP_STATUSES: return "Doorstep jobs use only Pending, Approved, Completed or Cancelled statuses"
        if values["status"] == "Approved" and not values["assigned_technician_id"]: return "Assign a technician before approving a Doorstep job"
    if values["payment_status"] not in PAYMENT_STATUSES: return "Invalid payment status"
    if not db.session.get(Customer, values["customer_id"]): return "Selected customer does not exist"
    if values["assigned_technician_id"] is not None and User.query.filter_by(id=values["assigned_technician_id"], role="technician").first() is None: return "Selected technician is invalid"
    payable = values["final_amount"] or values["estimated_amount"]
    if values["amount_paid"] > payable and payable > 0: return "Amount paid cannot exceed payable amount"
    return None
...
def _delivery_allowed(repair):
    ...
    if qc is None or qc.after_status not in {"Passed", "Waived"}: return False, "QC After Repair must be passed or waived before delivery"
    if repair.final_amount > 0 and repair.payment_status != "Paid": return False, "Invoice must be fully paid before delivery"
    return True, None
...
def _whatsapp_message(repair):
    ...
    values = {"customer_name": repair.customer.name, "job_number": repair.job_number, "device": repair.device, "status": repair.status, "amount": f"{repair.final_amount:.2f}", "amount_paid": f"{repair.amount_paid:.2f}", "payment_status": repair.payment_status}
...
def add_repair():
    customers = Customer.query.order_by(Customer.name.asc()).all(); technicians = User.query.filter_by(role="technician").order_by(User.email.asc()).all()
    if request.method == "POST":
        values = _repair_values(); error = _validate_repair(values)
        if error:
            flash(error, "error"); return render_template("repairs/form.html", customers=customers, technicians=technicians, action="Add")
...
def edit_repair(id):
    ...
    if request.method == "POST":
        values = _repair_values(); error = _validate_repair(values)
        if error:
            flash(error, "error"); return render_template("repairs/form.html", repair=repair, customers=customers, technicians=technicians, action="Edit")
...
def repairs_by_customer(customer_id):
    ...
    if g.current_user and g.current_user.role == "technician": repairs_query = repairs_query.filter(RepairOrder.assigned_technician_id == g.current_user.id)
    return render_template("repairs/customer_repairs.html", customer=customer, repairs=repairs_query.order_by(RepairOrder.created_at.desc()).all())
...
def update_repair_status(id):
    ...
    if repair.service_type == "Doorstep":
        if new_status not in DOORSTEP_STATUSES: flash("Doorstep jobs do not use Received, Diagnosing, In Repair, Ready or Delivered statuses", "error"); return redirect(url_for("repair.view_repair", id=id))
        if new_status == "Approved" and not repair.assigned_technician_id: flash("Assign a technician before approving a Doorstep job", "error"); return redirect(url_for("repair.view_repair", id=id))
    ...
    qc = RepairQC.query.filter_by(repair_id=repair.id).first()
    if repair.service_type != "Doorstep":
        if new_status == "In Repair" and (qc is None or qc.before_status not in {"Passed", "Waived"}): flash("QC Before Repair must be passed or waived before repair work can start", "error"); return redirect(url_for("repair.view_repair", id=id))
        if new_status == "Ready" and (qc is None or qc.after_status not in {"Passed", "Waived"}): flash("QC After Repair must be passed or waived before the repair can be marked Ready", "error"); return redirect(url_for("repair.view_repair", id=id))
    ...
    if old_status == new_status: flash("Repair order is already in this status", "success"); return redirect(url_for("repair.view_repair", id=id))
    repair.status = new_status
    if new_status == "Delivered" and repair.delivered_at is None: repair.delivered_at = datetime.now(timezone.utc)
    _audit(repair, "status_changed", old_status, new_status); db.session.commit(); flash("Repair order status updated", "success"); return redirect(url_for("repair.view_repair", id=id))
