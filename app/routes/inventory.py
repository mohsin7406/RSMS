...
from app.services.settings import get_bool, get_decimal, get_options
...
def _masters():return (InventoryCategory.query.filter_by(active=True).order_by(InventoryCategory.name).all(),Supplier.query.filter_by(active=True).order_by(Supplier.name).all())
@inventory_bp.route("/")
...
def list_parts():
    ...
    low_stock=[p for p in Part.query.filter_by(active=True).all() if p.quantity<=p.reorder_level];return render_template("inventory/list.html",parts=parts,low_stock=low_stock,search=search,stock_filter=stock)
@inventory_bp.route("/report")
@permission_required("inventory")
def inventory_report():
    parts=Part.query.filter_by(active=True).order_by(Part.name).all();low=[p for p in parts if p.quantity<=p.reorder_level];totals={"skus":len(parts),"units":sum((p.quantity for p in parts),Decimal("0")),"cost_value":sum((p.quantity*p.cost_price for p in parts),Decimal("0")),"retail_value":sum((p.quantity*p.selling_price for p in parts),Decimal("0")),"low_stock":len(low)};movements=StockMovement.query.order_by(StockMovement.created_at.desc()).limit(100).all();pending=[p for p in JobPurchase.query.order_by(JobPurchase.created_at.desc()).all() if p.remaining_quantity>0];returns=JobPurchaseReturn.query.order_by(JobPurchaseReturn.created_at.desc()).limit(100).all();return render_template("inventory/report.html",parts=parts,low_stock=low,totals=totals,movements=movements,pending_job_purchases=pending,returns=returns,returned_to_inventory=sum((r.quantity for r in returns if r.destination=="Inventory"),Decimal("0")),returned_to_supplier=sum((r.quantity for r in returns if r.destination=="Supplier"),Decimal("0")))
def _part_values(part):
    part.name=request.form.get("name","").strip();part.brand=request.form.get("brand","").strip() or None;part.model=request.form.get("model","").strip() or None;part.category=request.form.get("category","").strip() or None;raw=request.form.get("reorder_level","").strip();part.reorder_level=_decimal(raw) if raw else get_decimal("default_low_stock_threshold","2");part.cost_price=_decimal(request.form.get("cost_price"));part.selling_price=_decimal(request.form.get("selling_price"));part.supplier=request.form.get("supplier","").strip() or None
...
def adjust_stock(part_id):
    part=db.session.get(Part,part_id)
    if part is None or not part.active:abort(404)
    quantity=_decimal(request.form.get("quantity"));movement_type=request.form.get("movement_type","ADJUSTMENT");reason=request.form.get("reason","").strip();reasons=get_options("stock_adjustment_reasons")
    ...
    if reason and reasons and reason not in reasons:flash("Invalid stock adjustment reason","error");return redirect(url_for("inventory.list_parts"))
    if movement_type=="OUT" and part.quantity<quantity and not get_bool("allow_negative_stock"):flash("Insufficient stock","error");return redirect(url_for("inventory.list_parts"))
    part.quantity=part.quantity-quantity if movement_type=="OUT" else part.quantity+quantity;notes=" · ".join(filter(None,[reason,request.form.get("notes","").strip()])) or None;db.session.add(StockMovement(part_id=part.id,user_id=current_user_id(),movement_type=movement_type,quantity=quantity,reference=request.form.get("reference","").strip() or None,notes=notes));db.session.commit();flash("Stock updated","success");return redirect(url_for("inventory.list_parts"))
@inventory_bp.route("/masters",methods=["GET"])
@role_required("admin","manager")
def masters():return render_template("inventory/masters.html",categories=InventoryCategory.query.order_by(InventoryCategory.active.desc(),InventoryCategory.name).all(),suppliers=Supplier.query.order_by(Supplier.active.desc(),Supplier.name).all())
...
def save_supplier():
    ...
    row.name=name;row.phone=request.form.get("phone","").strip() or None;row.whatsapp=request.form.get("whatsapp","").strip() or None;row.gstin=request.form.get("gstin","").strip() or None;row.address=request.form.get("address","").strip() or None;row.notes=request.form.get("notes","").strip() or None;row.active=True;db.session.commit();flash("Supplier saved","success");return redirect(url_for("inventory.masters"))
